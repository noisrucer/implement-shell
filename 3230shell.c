 ...
 * Compile: gcc -o 3230shell 3230shell.c
 * Execute: ./3230shell
 * Remarks
        ...
        for(idx = 0; idx < totalNumCmds; idx++) {
            ...
            if (pid == 0) {
                ...
                if (useBg == 0){
                    ...
                    else if (WIFSIGNALED(status)) { // Handling the child process terminated by a signal
...
char* trim(char* str) {
     ...
     * */
    if (isEmpty(str) || isAllSpaces(str)) return str; // if empty or consists of only spaces, return it
